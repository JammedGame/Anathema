...
{  
    ...
    public Update() : void
    {
        ...
        for(let i = 0; i < BackPackY; i++)
        {
            for(let j = 0; j < BackPackX; j++)
            {
                ...
                {
                    this.CreateInventoryItem(i * BackPackX + j, "BackPack", this._Inventory.BackPack[i * BackPackX + j], new Engineer.Vertex(1400 + j * 50, 590 + i * 50,0));
                }
            }
        }
    ...
    public InitElements() : void
    {
        ...
        this.Trans.Translation = new Engineer.Vertex(1600, 460, 2);
        this.CreateBorder();
        let Head:any = this.AddElement(new Engineer.Vertex(220,80,2.5), new Engineer.Vertex(50,50,1), 0);
        this.Data["HeadSlot"] = Head;
        Head.Data["Slot"] = true;
        Head.Data["SlotType"] = "Head";
        Head.Events.MouseUp.push(this.SlotMouseUp.bind(this));
        let Weapon:any = this.AddElement(new Engineer.Vertex(120,170,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        let OffHand = this.AddElement(new Engineer.Vertex(320,170,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        let Chest = this.AddElement(new Engineer.Vertex(220,170,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        let Gloves = this.AddElement(new Engineer.Vertex(120,260,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        let Greaves = this.AddElement(new Engineer.Vertex(220,260,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        let Boots = this.AddElement(new Engineer.Vertex(220,350,2.5), new Engineer.Vertex(50,50,1), 0);
        ...
        for(let i = 0; i < BackPackY; i++)
        {
            for(let j = 0; j < BackPackX; j++)
            {                  
                let Slot = this.AddElement(new Engineer.Vertex(j * 50, 480 + i * 50, 2.5), new Engineer.Vertex(50,50,1), 0);
                Slot.Data["Slot"] = true;
    ...
    private SlotMouseUp(G:any, Args:any)
    {
        if(this._Dragged)
        {
            ...
            else if(Sender.Data["SlotType"] != "BackPack" && Sender.Data["SlotType"] == this._Dragged.Item.Data["Type"])
            {
                ...
                this._Dragged.Trans.Scale = new Engineer.Vertex(80,80,1);
                this._Dragged.Trans.Translation = Sender.Trans.Translation;                
    ...
    private ItemDragStart(G:any, Args:any)
    {
        ...
        this._Dragged.Trans.Scale = new Engineer.Vertex(60,60,1);
    }
    private MouseMove(G:any, Args:any)
    {
        ...
        if(this._Dragged)
        {
            this._Dragged.Trans.Translation = new Engineer.Vertex(Args.Location.X, Args.Location.Y, this._Dragged.Trans.Translation.Z);
        }
        let OverlayTarget = false;
        for(let i = 0; i < this._InventoryItems.length; i++)
        {
            if(Engineer.Vertex.Distance(this._InventoryItems[i].Trans.Translation, Args.Location) < 60)
            {
