...
class LevelGenerator {
    ...
    public static generate(scene: GameScene, level: Level): void {
        ...
        level.layout = LevelGenerator.GenerateLayout(new TBX.Vertex(level.dimensions[0], level.dimensions[0], 0), [new LayoutClass(3, 1), new LayoutClass(2, 3), new LayoutClass(1, 1)]);
    ...
    private static GenerateMegaChunk(L: Layout, tileset: LevelTileset): Chunk {
        let MC: Chunk = new Chunk(new TBX.Vertex(L.Dimensions.X * 11 - 1, L.Dimensions.Y * 11 - 1, 0), -1);
        for (let i = 0; i < L.Entries.length; i++) {
            let Index = Math.floor((Math.random() * 4));
            if (Index == 4) Index = 3;
            let NewChunk: Chunk = LevelGenerator._ChunkGenerator.GenerateRandom(new TBX.Vertex(L.Entries[i].Size * 11 - 1, L.Entries[i].Size * 11 - 1, 0));
            L.Entries[i].Chunk = NewChunk;
            LevelGenerator._ChunkGenerator.Insert(MC, NewChunk, new TBX.Vertex(L.Entries[i].Location.X * 11, L.Entries[i].Location.Y * 11, 0));
        }
        LevelGenerator.ConnectMegaChunk(MC, L);
        LevelGenerator._ChunkGenerator.FakeIsometric(MC, tileset.wallVoid);
        if (tileset.settings.ceiling == LevelTilesetCeilingType.Roofed) LevelGenerator._ChunkGenerator.FakeRoof(MC);
        return MC;
    }
