...
class LevelGenerator
{
    ...
    {
        let MC:Chunk = new Chunk(new Engineer.Vertex(L.Dimensions.X * 11 - 1, L.Dimensions.Y * 11 - 1, 0), -1);
        for(let i = 0; i < L.Entries.length; i++)
        {
            let Index = Math.floor((Math.random() * 4));
            if(Index == 4) Index = 3;
            let NewChunk:Chunk = LevelGenerator._ChunkGenerator.GenerateRandom(new Engineer.Vertex(L.Entries[i].Size * 11 - 1, L.Entries[i].Size * 11 - 1, 0));
            L.Entries[i].Chunk = NewChunk;
            LevelGenerator._ChunkGenerator.Insert(MC, NewChunk, new Engineer.Vertex(L.Entries[i].Location.X * 11, L.Entries[i].Location.Y * 11, 0));
        }
    ...
    {
        let Location = new Engineer.Vertex(0,0,0);
            ...
            for(let j = 0; j < LayoutClasses[i].Number; j++)
            {
                ...
                for(let k = 0; k < Dimensions.Y - LayoutClasses[i].Size + 1; k++)
                {
                    for(let l = 0; l < Dimensions.X - LayoutClasses[i].Size + 1; l++)
                    {
                        ...
                        if(EntryAvailable) Available.push(new LayoutEntry(LayoutClasses[i].Size, new Engineer.Vertex(l,k,0)));
                    }   
                }
