...
import { LevelTilesetCeilingType, LevelTilesetFillType } from '../../Tilesets/LevelTilesetBlueprint';
...
class EnvironmentCeilingGenerator {
    public static Generate(level: Level, Art: Chunk): void {
        ...
        for (let i = 0; i < C.Dimensions.Y; i++) {
            for (let j = 0; j < C.Dimensions.X; j++) {
                if (C.Fields[i][j] == 4 || ((C.Fields[i][j] == 0 || C.Fields[i][j] == -1) && level.tileset.settings.fill == LevelTilesetFillType.Ceiling)) {
    ...
    private static GenerateCeiling(level: Level, C: Chunk, X: number, Y: number): number {
        if (level.tileset.settings.ceiling == LevelTilesetCeilingType.Uniform) return EnvironmentCeilingGenerator.GenerateUniform();
        if (level.tileset.settings.ceiling == LevelTilesetCeilingType.Roofed) return EnvironmentCeilingGenerator.GenerateRoofed(C, X, Y);
        if (level.tileset.settings.ceiling == LevelTilesetCeilingType.Crested) return EnvironmentCeilingGenerator.GenerateCrested(C, X, Y);
        if (level.tileset.settings.ceiling == LevelTilesetCeilingType.Bordered) return EnvironmentCeilingGenerator.GenerateBordered(C, X, Y);
        if (level.tileset.settings.ceiling == LevelTilesetCeilingType.Divided) return EnvironmentCeilingGenerator.GenerateCrested(C, X, Y);
        return -1;
    }
...
export default EnvironmentCeilingGenerator;
